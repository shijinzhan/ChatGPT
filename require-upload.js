    ...
    events: {
      ...
      onUploadSuccess: function(up, ret, file) {
        ...
        if (button) {
          ...
          if (onDomUploadSuccess) {
            if (typeof onDomUploadSuccess !== 'function' && typeof Upload.api.custom[onDomUploadSuccess] ===
              'function') {
      ...
      onUploadError: function(up, ret, file) {
        ...
        if (button) {
          ...
          if (onDomUploadError) {
            if (typeof onDomUploadError !== 'function' && typeof Upload.api.custom[onDomUploadError] ===
              'function') {
      ...
      onUploadComplete: function(up, files) {
        ...
        if (button) {
          ...
          if (onDomUploadComplete) {
            if (typeof onDomUploadComplete !== 'function' && typeof Upload.api.custom[onDomUploadComplete] ===
              'function') {
    ...
    api: {
      ...
      upload: function(element, onUploadSuccess, onUploadError, onUploadComplete) {
        ...
        $(element, Upload.config.container).each(function() {
          ...
          var category = typeof params.category !== 'undefined' ? params.category : ($(this).data(
            "category") || '');
          if (category) {
          ...
          Upload.list[id] = new Dropzone(this, $.extend({
            ...
            dictFileTooBig: __("File is too big (%sMiB), Max filesize: %sMiB", "{{filesize}}",
              "{{maxFilesize}}"),
            ...
            addedfile: function(file) {
              ...
              var category = typeof params.category !== 'undefined' ? params.category : ($(this
                .element).data("category") || '');
              file.category = typeof category === 'function' ? category.call(this, file) : category;
            },
            addedfiles: function(files) {
              if (this.options.maxFiles && (!this.options.maxFiles || this.options.maxFiles > 1) &&
                ...
                if (inputObj.length > 0) {
                  ...
                  if (remain === 0 || files.length > remain) {
                    ...
                    Toastr.error(__("You can only upload a maximum of %s files", this.options
                      .maxFiles));
                  }
                }
              }
            ...
            error: function(file, response, xhr) {
              var responseObj = $("<div>" + (xhr && typeof xhr.responseText !== 'undefined' ? xhr
            ...
            uploadprogress: function(file, progress, bytesSent) {
              if (file.upload.chunked) {
                $(this.element).prop("disabled", true).html("<i class='fa fa-upload'></i> " + __(
                  'Upload') + Math.floor((file.upload.bytesSent / file.size) * 100) + "%");
              }
            },
          ...
          if (preview_id && input_id) {
            $(document.body).on("keyup change", "#" + input_id, function(e) {
              ...
              "";
              var extend = $("#" + preview_id).next().is("textarea") ? $("#" + preview_id).next(
                "textarea").val() : "{}";
              var json = {};
              try {
              ...
              $.each(inputArr, function(i, j) {
                ...
                var html = tpl ? Template(tpl, data) : Template.render(Upload.config.previewtpl,
                  data);
          ...
          if (input_id) {
            ...
            $("body").on('paste drop', "#" + input_id, function(event) {
              ...
              var button = $(".plupload[data-input-id='" + $(this).attr("id") +
                "'],.faupload[data-input-id='" + $(this).attr("id") + "']");
              if (event.type === 'paste' && originEvent.clipboardData && originEvent.clipboardData
                .items) {
                var items = originEvent.clipboardData.items;
                if ((items.length === 1 && items[0].type.indexOf("text") > -1) || (items.length === 2 &&
                    items[1].type.indexOf("text") > -1)) {
